...
<div class="stage">
  <div class="stage-inner">
    ...
    <div class="sub no-print">Konfirmasi pendaftaran dan tiket QR Anda telah aktif. Silakan download tiket ini untuk akses masuk event.</div>
    ...
    <div class="ticket" id="ticket-content">
      <div class="ticket-top">
        ...
        @if($event->speaker)<div style="font-size:13px; color:rgba(255,255,255,0.8); margin-top:6px;">Pemateri: {{ $event->speaker }}</div>@endif
      </div>
...
function downloadTicket() {
  var content = document.getElementById('ticket-content').cloneNode(true);
  var qrImg = content.querySelector('#ticket-qr canvas');
  if (qrImg) {
    var img = document.createElement('img');
    img.src = qrImg.toDataURL('image/png');
    img.style.width = '100px';
    img.style.height = '100px';
    content.querySelector('#ticket-qr').innerHTML = '';
    content.querySelector('#ticket-qr').appendChild(img);
  }

  var style = document.createElement('style');
  style.textContent = `
    body { font-family: Arial, sans-serif; padding: 40px; background: #fff; color: #000; }
    .ev { font-size: 24px; font-weight: 800; margin-bottom: 8px; }
    .dt { font-size: 14px; color: #555; margin-bottom: 6px; }
    .stub-divider { border-top: 2px dashed #ccc; margin: 24px 0; }
    .ticket-mid { display: flex; align-items: center; gap: 24px; flex-wrap: wrap; }
    .qr-holder { background: #fff; padding: 10px; border-radius: 8px; display: inline-block; border: 1px solid #eee; }
    .info { flex: 1; }
    .name { font-size: 20px; font-weight: 800; margin-bottom: 8px; }
    .code { font-size: 14px; margin-bottom: 6px; }
    .badge { display: inline-block; padding: 4px 10px; border-radius: 999px; font-size: 12px; font-weight: 700; }
    .ticket-bottom { margin-top: 20px; padding-top: 16px; border-top: 1px dashed #ccc; font-size: 12px; color: #666; }
    .mono { font-family: monospace; }
  `;

  var html = '<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>E-Tiket {{ $participant->trx_id }}</title>' + style.outerHTML + '</head><body>' + content.outerHTML + '</body></html>';

  var blob = new Blob([html], { type: 'text/html' });
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  a.href = url;
  a.download = 'tiket-{{ $participant->trx_id }}.html';
